...
class ArxivApiClient:
    ...
    def search(self, query: str, category: str = None, start_date: datetime = None,
        ...
        try:
            ...
            url = f"{self.base_url}?search_query={search_query}&max_results={max_results}&sortBy={sort_by}&sortOrder={sort_order}"
            ...
            if response.status_code != 200:
                raise Exception(f"API request failed with status {response.status_code}: {response.content}")
            ...
            for entry in entries:
                if isinstance(entry, dict):  # JSON entry
                    ...
                    paper = {
                        ...
                        'title': self._get_xml_text(entry, "atom:title", ns),
                        'summary': self._get_xml_text(entry, "atom:summary", ns),
